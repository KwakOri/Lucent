...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!emailRegex.test(email)) {
      return handleApiError(new Error('올바른 이메일 형식을 입력해주세요'), 400);
    }

    // 비밀번호 길이 검증
    if (password.length < 6) {
      return handleApiError(new Error('비밀번호는 최소 6자 이상이어야 합니다'), 400);
    }
    ...
    if (!canResend) {
      return handleApiError(
        new Error('이메일 재발송은 60초 후에 가능합니다. 잠시 후 다시 시도해주세요.'),
        429
      );
    }
    ...
    await LogService.log({
      category: 'auth',
      action: 'send_verification',
      description: `인증 코드 발송 실패: ${error.message}`,
