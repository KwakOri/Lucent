...
export async function GET(
  ...
  try {
    ...
    const { data: orderItems, error: orderError } = await supabase
      ...
      .in('orders.status', ['입금완료', '발송준비', '발송완료', '구매확정'])
      .in('item_status', ['입금완료', '다운로드가능', '발송준비', '발송완료', '구매확정']);
    ...
    if (!orderItems || orderItems.length === 0) {
      console.warn('[Download] 구매 내역 없음:', { userId: user.id, productId });
    ...
    console.log('[Download] Google Drive 링크로 리다이렉트:', product.digital_file_url);
