...
Options:
  ...
  --variant-id <uuid>             PHYSICAL 검증 variant_id (없으면 가격 적용 가능한 ACTIVE+PHYSICAL 1건 자동 선택)
  --digital-variant-id <uuid>     DIGITAL 검증 variant_id (없으면 가격 적용 가능한 ACTIVE+DIGITAL 1건 자동 선택 시도)
...
where v.status = 'ACTIVE'
  ...
  and v.fulfillment_type = 'PHYSICAL'
  and exists (
    select 1
...
where v.status = 'ACTIVE'
  and v.deleted_at is null
  and v.fulfillment_type = 'DIGITAL'
  and exists (
    select 1
    from public.v2_price_list_items pli
    join public.v2_price_lists pl
...
if [[ -z "${VARIANT_ID}" ]]; then
  echo "가격 적용 가능한 active physical variant를 찾지 못했습니다. 'npm run ops:v2-seed-dummy:local' 실행 후 재시도하거나 --variant-id를 직접 지정하세요." >&2
  exit 1
fi
